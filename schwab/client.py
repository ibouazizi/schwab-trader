...
from .models.generated.trading_models import (
    ...
    ComplexOrderStrategyType, OrderStrategyType, OrderLegCollection, OrderLegType,
    ...
    def _make_request(
        ...
        if response.status_code >= 400:
            ...
            raise _rq.exceptions.HTTPError(
                f"{response.status_code} {response.reason} for url: {response.url} — body: {response.text[:2000]}",
                response=response,
            )
        ...
        try:
            ...
            # Empty body (e.g. 201 Created from place_order / DELETE 204) — nothing to parse
